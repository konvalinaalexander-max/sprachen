/* Bühne "La noche" – Dialogmotor und 2D-Kulisse.
   Der Dialogmotor (L.nocheDialogo) ist von der Kulisse getrennt: die
   3D-Welt "barrio" nutzt denselben Motor mit einer anderen Kulisse.
   Ohne WebGL fällt "barrio" auf diese 2D-Fassung zurück. */
(function (L) {
  'use strict';
  var h = L.h, esc = L.esc;

  /* ---------------------------------------------------------------
     Dialogmotor: HUD, Panel, Szenenlogik, Bewertung.
     hooks.onPortada(), hooks.onScene(i, t, escena), hooks.onFinal(onda)
     --------------------------------------------------------------- */
  L.nocheDialogo = function (lesson, root, hooks) {
    var S = L.session;
    ...
    function pintarOnda() {
      ...
      for (var i = 0; i < 5; i++) hOnda.appendChild(h('i', { class: i < S.stage.onda ? 'on' : '' }));
    }

    function portada() {
      if (hooks.onPortada) hooks.onPortada();
      hud.style.opacity = '0';
    ...
    function pintar() {
      ...
      if (hooks.onScene) hooks.onScene(i, t, e);
      pintarOnda();
    ...
    function marcar(e, ok, mio, correcto) {
      ...
      if (hooks.onAnswer) hooks.onAnswer(ok);
      L.stageAnswer({
    ...
    function reaccion(cuerpo, ok, e, correcto) {
    ...
    function formas(e, cuerpo) {
      ...
      L.shuffle(e.formas.slice()).forEach(function (f) {
        ...
        b.addEventListener('click', function () {
          ...
          if (!f.ok && correcta) setTimeout(function () { hueco.textContent = correcta.t; hueco.classList.remove('mal'); hueco.classList.add('bien'); }, 900);
          if (f.why) cuerpo.appendChild(h('p', { class: 'porque', html: L.markup(f.why) }));
          marcar(e, !!f.ok, f.t, correcta ? correcta.t : '');
          reaccion(cuerpo, !!f.ok, e, f.ok ? '' : (correcta ? correcta.t : ''));
        });
        caja.appendChild(b);
      });
      cuerpo.appendChild(caja);
    ...
    function escribir(e, cuerpo) {
      ...
      function enviar() {
        ...
        if (!r.ok) {
          cuerpo.appendChild(h('div', { class: 'diff porque', html: L.diff(mio, [].concat(e.respuesta)[0]).map(function (x) {
        ...
        reaccion(cuerpo, r.ok, e, r.ok ? '' : [].concat(e.respuesta)[0]);
      }
    ...
    function final() {
      if (hooks.onFinal) hooks.onFinal(S.stage.onda);
      hud.style.opacity = '0';
      panel.classList.add('centro');
      panel.innerHTML = '';
      var onda = S.stage.onda;
      var txt = (lesson.finales && lesson.finales[Math.min(5, Math.max(0, onda))]) || '';
    ...
    return { hud: hud, panel: panel, pintar: pintar };
  };

  /* ---------------------------------------------------------------
     2D-Kulisse: Himmel, Skyline, Balkon
     --------------------------------------------------------------- */
  var SKY = {
  ...
  function skylineFar() {
    return '<svg class="sky-far" viewBox="0 0 1200 240" preserveAspectRatio="none" aria-hidden="true">' +
      ...
      'L968,138 L1036,138 L1036,160 L1104,160 L1104,150 L1140,150 L1140,168 L1200,168 L1200,240 Z"/></svg>';
  }
  function skylineNear() {
    var win = '';
    for (var r = 0; r < 3; r++) for (var c = 0; c < 26; c++) {
      win += '<rect class="win" x="' + (40 + c * 44) + '" y="' + (96 + r * 40) + '" width="17" height="23" rx="2"/>';
    }
    return '<svg class="sky-near" viewBox="0 0 1200 260" preserveAspectRatio="none" aria-hidden="true">' +
      ...
      '<g class="antennas"><path d="M300,84 L300,44 M292,54 L308,54 M294,64 L306,64"/>' +
      '<path d="M880,84 L880,38 M872,48 L888,48"/><circle cx="600" cy="66" r="13" class="dish"/><path d="M600,79 L600,84"/></g>' +
      '<g class="tendedero"><path d="M120,92 Q300,116 480,92"/><rect x="196" y="104" width="20" height="26" rx="2"/>' +
      '<rect x="248" y="108" width="26" height="20" rx="2"/><rect x="320" y="110" width="18" height="28" rx="2"/>' +
      '<rect x="388" y="106" width="24" height="22" rx="2"/></g><g class="wins">' + win + '</g></svg>';
  }
  function balcony() {
    var bars = '';
    ...
    return '<svg class="balcon" viewBox="0 0 1200 120" preserveAspectRatio="none" aria-hidden="true">' +
      '<g class="rail">' + bars + '<path d="M0,8 L1200,8"/><path d="M0,52 L1200,52"/><path d="M0,96 L1200,96"/></g>' +
      '<g class="maceta"><path d="M980,96 L980,58 L1052,58 L1046,96 Z"/><path d="M990,58 q6,-34 26,-30 q-4,20 -26,30 Z"/>' +
      '<path d="M1020,58 q-2,-30 22,-26 q0,20 -22,26 Z"/></g></svg>';
  }
  ...
  L.stages.noche = function (lesson) {
    ...
    for (var s = 0; s < 60; s++) {
      stars.appendChild(h('i', { style: 'left:' + (Math.random() * 100).toFixed(2) + '%;top:' + (Math.random() * 62).toFixed(2) +
    ...
    function ambiente(t) {
      ...
      near.querySelectorAll('.win').forEach(function (w, i) { w.classList.toggle('lit', (i * 7 + 3) % 78 < lit); });
      root.style.setProperty('--glow', L.ramp(['#ffb066', '#ff8a5c', '#b06a8a', '#5b6ba8'], t));
    }

    L.nocheDialogo(lesson, root, {
      onPortada: function () { ambiente(0); },
      onScene: function (i, t) { ambiente(t); },
      onFinal: function () { ambiente(1); }
    });
    return root;
  };
